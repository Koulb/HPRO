"""
Direct real-space Band-RI reconstruction for Diamond 2x2x2 supercell.
...
# =============================================================================
print("=" * 70)
print("Diamond 2x2x2 Supercell: Direct Real-Space Band-RI Reconstruction")
print("=" * 70)

# Paths (from aohamiltonian dir)
...
# Generate full canonical R grid
canonical_R_set = set()
...
band_counts = [20, 40, 60, 80, 100, 150, 200, 300, 400, 500, 600, 800]
errors_vs_R0 = []
...
for ik in range(len(valid_kpts)):
    ...
    # Band-RI: direct H(k) = A†(k) diag(ε) A(k)
    eigs_k = valid_eigs[ik][:max_nbands]
    A_0_k = A_0_list[ik]
    eA = eigs_k[:, None] * A_0_k
...
print(f"\n{'Target bands':>14} {'MAE Original (meV)':>20} {'MAE Band-RI (meV)':>20}")
print("-" * 56)
for n in target_bands:
    print(f"{n:14d} {np.mean(mae_original[n]):20.1f} {np.mean(mae_bandri[n]):20.1f}")
...
print("=" * 70)
print(f"""
Diamond 2x2x2 supercell direct real-space Band-RI reconstruction:

1. H(R=0) convergence (Band-RI vs Original):
   - 20 bands: {errors_vs_R0[0]*100:.1f}% error
   - 100 bands: {errors_vs_R0[4]*100:.1f}% error
   - 200 bands: {errors_vs_R0[-1]*100:.1f}% error
